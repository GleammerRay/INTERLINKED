...
class Command {
  ...
  async execute(interaction) {
    return await this.rest.createInteractionResponse(interaction, { type: 4, data: {
        embeds: [
          {
            title: 'Player queue',
            footer: {
            ...
            author: { name: 'INTERLINKED', url: 'https://github.com/GleammerRay/INTERLINKED', icon_url: this.logoURL },
            description: 'Join the player queue to get notified once enough interest is gathered.',
          },
        ],
        components: [
          {
  ...
  async executeButton(interaction) {
    ...
    if (shouldSave) {
      ...
      for (const playerId in playerQueue1.players) {
        ...
        if (player.interest <= 5) {
          if (interest5 >= 5 || interest10 >= 10 || interest15 >= 15) {
            playersToPing.push(playerId);
            player.pinged = true;
          }
        }
        else if (player.interest <= 10) {
          if (interest10 >= 10 || interest15 >= 15) {
            playersToPing.push(playerId);
            player.pinged = true;
          }
        }
        else if (player.interest <= 15) {
          if (interest15 >= 15) {
            playersToPing.push(playerId);
            player.pinged = true;
          }
      ...
      if (playersToPing.length != 0) {
        const message = `> **Player queue ${arg1 + 1} is ready** - join up! https://discord.com/channels/${interaction.guild_id}/${interaction.channel_id}`;
