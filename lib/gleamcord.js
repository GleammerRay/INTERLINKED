...
export class REST {
  ...
  async sendRequest(url, method, body) {
    ...
    while (true) {
      ...
      if (this.#header != null) _xhr.setRequestHeader('Authorization', this.#header);
  ...
  getGuild(guildID) {
    return this.get(`${restGuildsURL}/${guildID}`);
  }

  getGuildMember(guildID, userID) {
    return this.get(`${restGuildsURL}/${guildID}/members/${userID}`);
  }
  
  getGuildRoles(guildID) {
    return this.get(`${restGuildsURL}/${guildID}/roles`);
  }
  ...
  startThread(channelID, name, autoArchiveDuration = null, type = null, invitable = null, rateLimitPerUser = null) {
    return this.post(`${restChannelsURL}/${channelID}/threads`, `{"name":"${name}","auto_archive_duration":${autoArchiveDuration},"type":${type},"invitable":${invitable},"rate_limit_per_user":${rateLimitPerUser}}`);
  }

  startThreadFromMessage(channelID, messageID, name, autoArchiveDuration = null, rateLimitPerUser = null) {    
    return this.post(`${restChannelsURL}/${channelID}/messages/${messageID}/threads`, `{"name":"${name}","auto_archive_duration":${autoArchiveDuration},"rate_limit_per_user":${rateLimitPerUser}}`);
  }
  ...
  createEphemeralInteractionResponse(interaction, response) {
    if (typeof response == 'string') response = JSON.parse(response);
    response.flags = 1 << 6;
    return this.post(`${restInteractionsURL}/${interaction.id}/${interaction.token}/callback`, `{"type":4,"data":${JSON.stringify(response)}}`);
  }
  
  updateInteractionResponse(interaction, response) {
    if (typeof response != 'string') response = JSON.stringify(response);
    return this.post(`${restInteractionsURL}/${interaction.id}/${interaction.token}/callback`, `{"type":7,"data":${response}}`);
  }

  createCompletionInteractionResponse(interaction, value, choices) {
    choices = [...choices];
    choices.sort((a, b) => simpleStringMatchAccuracy(b.toLowerCase(), value.toLowerCase()) - simpleStringMatchAccuracy(a.toLowerCase(), value.toLowerCase()));
    var fmtChoices = [];
    choices.forEach((name) => fmtChoices.push({ name: name, value: name }));
    return this.post(`${restInteractionsURL}/${interaction.id}/${interaction.token}/callback`, `{"type":8,"data":{"choices":${JSON.stringify(fmtChoices)}}}`);
  }
  ...
  bulkOverwriteGlobalApplicationCommands(applicationID, commands) {
    if (typeof commands != 'string') commands = JSON.stringify(commands);
