...
export class InterlinkedServer {
  ...
  constructor(address, name, mapName, players, maxPlayerCount) {
  ...
  static fromSteamServer(server) {
    ...
    var players = server.playerInfo.players;
    return new InterlinkedServer(server.addr, server.name, server.map, server.playerInfo.players, server.max_players);
  }
}
...
export class InterlinkedGuild {
  ...
  buildServerListMessage(serverList, image = null, showInactive = false, components = serverListComponents) {
    ...
    if (showInactive) {
      var _serverAddrs = Object.keys(serverList.servers);
      var _servers = Object.values(serverList.servers);
      for (var i = 0; (i != _serverAddrs.length) && (_fields.length != 15); i++) {
...
export class InterlinkedDiscord {
  ...
  getOrSetGuild(guildID) {
    if (!(guildID in this.#guilds)) {
      var _guild = new InterlinkedGuild(guildID, this.#rest, this.#gameName, this.#images, this.#onLog, defaultMaxServerCount, null, [], defaultSubscriptionFeedMinRate, defaultMinPlayerCount, null, defaultPingMinRate, defaultPingMinPlayerCount, [], null, null, this.logoURL);
...
export class Interlinked {
  ...
  constructor(
    ...
    if (topServersHTMLStylePath != null) {
