...
'{' +
  `"name":"remove","type":1,"description":"${removeDesc}","default_member_permissions":"0","default_permission":false,"dm_permission":false,"options":[` +
    ...
    `{"name":"admin_role","type":1,"default_member_permissions":"0","default_permission":false,"dm_permission":false,"description":"${adminRoleDesc}"}` +
  ']' +
'},' +
'{' +
...
export class InterlinkedGuild {
  ...
  _getBlacklistedServerList(serverList) {
    ...
    for (var i = 0; i != serverList.activeServers.length; i++) {
      ...
      if (!this.#blacklist.includes(_server.address)) {
        ...
        if (_serverList.serverCount >= this.maxServerCount) break; 
        continue;
      }
    }
    return _serverList;
  ...
  removeVariable(interaction) {
    ...
    this[_varName] = null;
    return this.#rest.createInteractionResponse(interaction, `{"type":4,"data":{"content":"> \`${_varName}\` = null :bomb:"}}`);
  }
...
export class Interlinked {
  ...
  async getServers() {
    ...
    if (typeof _servers == 'string') {
      this._onLog(`WARNING:Received unknown data from steam:\n${_servers}`);
      return [];
    }
