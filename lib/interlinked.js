...
'{' +
  `"name":"get","type":1,"description":"${getDesc}","dm_permission":false,"options":[` +
    ...
    `{"name":"blacklist","type":1,"description":"${getBlacklistDesc}"}` +
  ']' +
'},' +
'{' +
...
class InterlinkedImages {
  ...
  constructor(activeImageURLs = [], fridayImageURLs = []) {
...
export class InterlinkedGuild {
  ...
  buildServerListMessage(serverList, image = this.getImage()) {
  ...
  async postServerListInteractionResponse(interaction, serverList, image = this.getImage()) {
  ...
  postServerListMessage(channelID, serverList, image = this.getImage()) {
    if (serverList == null) return;
  ...
  async postSubscriptionFeed(serverList, image = this.getImage()) {
...
export class Interlinked {
  ...
  constructor(
    ...
    this.#onLog = onLog;
    this.activeImageURLs = config.activeImageURLs;
    this.fridayImageURLs = config.fridayImageURLs;
    this.#isRunning = false;
    this.#botToken = config.discordBotToken;
    this.#rest = REST.fromBotToken(config.discordBotToken);
    this.#gateway = null;
    this.#discord = InterlinkedDiscord.fromJSON(discordPrefs, this.#rest, config.gameName, new InterlinkedImages(config.activeImageURLs, config.fridayImageURLs), onSave, onLog);
