...
export class InterlinkedGuild {
  ...
  set minPlayerCount(value) {
    ...
    this.#isDelayNextSubscriptionFeedUpdateActive = false;
  }
  get minPlayerCount() {
  ...
  set pingRole(value) {
    this.#pingRole = value;
    this.#canPing = true;
    this.#isDelayNextPingActive = false;
  }
  get pingRole() {
  ...
  #isDelayNextSubscriptionFeedUpdateActive;
  #isDelayNextPingActive;
  ...
  static fromJSON(json, rest, gameName, onLog) {
  ...
  _onTimeoutDelayNextSubscriptionFeedUpdate() {
    this.#isDelayNextSubscriptionFeedUpdateActive = false;
    this.#canPostSubscriptionFeed = true;
  }
  
  delayNextSubscriptionFeedUpdate() {
    if (this.#isDelayNextSubscriptionFeedUpdateActive) return;
    this.#canPostSubscriptionFeed = false;
    this.#isDelayNextSubscriptionFeedUpdateActive = true;
    setTimeout(() => _onTimeoutDelayNextSubscriptionFeedUpdate(), this.#subscriptionFeedMinRate);
  }
  
  _onTimeoutDelayNextPing() {
    this.#isDelayNextPingActive = false;
    this.#canPing = true;      
  }
  
  delayNextPing() {
    if (this.#isDelayNextPingActive) return;
    this.#canPing = false;
    this.#isDelayNextPingActive = true;
    setTimeout(() => _onTimeoutDelayNextPing(), this.#pingMinRate);
  }
  ...
  async postServerListInteractionResponse(interaction, serverList, image) {
    ...
    this._onLog(`INFO:Sending server list to channel ${interaction.channel_id}.`);
    if (this.shouldPing(serverList)) {
  ...
  postServerListMessage(channelID, serverList, image) {
    ...
    var _message = this.buildServerListMessage(serverList, image);
    this._onLog(`INFO:Sending server list to channel ${channelID}.`);
...
export class InterlinkedDiscordJSON {
  ...
  static fromJSON(json) {
    return InterlinkedDiscordJSON(json.guilds);
  }
}
...
export class InterlinkedDiscord {
  ...
  constructor(guilds, gameName, rest, onSave, onLog) {
  ...
  static fromJSON(json, gameName, rest, onSave, onLog) {
    if (typeof rest == 'string') rest = REST.fromBotToken(rest);
    var _guilds = {};
  ...
  async addUserPingRole(interaction) {
    this._onLog(`INFO:Adding ping role to user ${interaction.member.user.id} in guild ${interaction.guild_id}`);
...
export class Interlinked {
  ...
  constructor(
    ...
    this.#serverRequestURL = Interlinked.getSteamServerRequestURL(steamAPIKey, steamAppID);
    this.#servers = [];
  }
  ...
  static getSteamServerRequestURL(steamAPIKey, steamAppID) {
  ...
  set steamAPIKey(value) {
    ...
    this.#serverRequestURL = Interlinked.getSteamServerRequestURL(this.#steamAPIKey, this.#steamAppID);
  }
  ...
  async updateSteamServers() {
    this.#servers = InterlinkedServerList.fromSteamServers((await this.getSteamServers()).response.servers);
