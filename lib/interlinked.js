...
export class InterlinkedGameList {
  ...
  constructor(name = '', websiteURL = '', discordURL = '', logoURL = null, color = null, games = []) {
    this.name = name;
  ...
  generateDiscordMessages() {
    ...
    for (let i = 0; i != this.games.length; i++) {
      ...
      messages[messageIndex].embeds[0].fields.push(field)
      fieldIndex++;
      if (fieldIndex == 15) {
        ...
        var message = {
          embeds: [
            {
              title: `Games list (Page ${messageIndex + 1})`,
              fields: [],
              color: this.color,
              timestamp: new Date().toISOString(),
            }
