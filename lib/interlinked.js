...
const defaultGameListURL = 'https://raw.githubusercontent.com/UndeadGames/undeadgames.github.io/test/undeadgames.json';
...
export class InterlinkedGame {
  ...
  constructor(name, logoURL, description, downloadPage, activity, connections = []) {
    this.name = name;
  ...
  static fromJSON(json) {
    ...
    return new InterlinkedGame(name, logoURL, description, downloadPage, activity, connections);
  }
}
...
export class InterlinkedGameList {
  ...
  generateDiscordMessages() {
    ...
    messages[0].embeds[0].fields = [];
    for (let i = 0; i != this.games.length; i++) {
      var game = this.games[i];
      var gameName = `🎮 ${game.name}`;
      if (gameName.length > 100) gameName = gameName.slice(0, 64);  
      var field = {
        name: gameName,
        value: `• Description: ${game.description}`,
      };
      if (game.downloadPage != null) field.value += `\n\n• Download page: ${game.downloadPage}`;
      if (game.activity != null) field.value += `\n\n• Activity: ${game.activity}`;
      if (game.connections != null) if (game.connections.length != 0) {
        for (let j = 0; j != game.connections.length; j++) {
          var connection = game.connections[j];
          field.value += `\n\n• ${connection.name}: ${connection.value}`;
        }
      }
      if (field.value.length > 800) field.value = field.value.slice(0, 800);
      messages[messages.length - 1].embeds[0].fields.push(field)
      fieldIndex++;
      if (fieldIndex == 5) {
        ...
        var message = {
          embeds: [
            {
              title: `Games list (Page ${messages.length + 1})`,
              fields: [],
              color: this.color,
              timestamp: new Date().toISOString(),
              image: { url: 'https://raw.githubusercontent.com/UndeadGames/undeadgames.github.io/main/assets/images/423x1.png' }
            }
          ],
        };
    ...
    messages[messages.length - 1].embeds[0].image = image;
    return messages;
  }
}
...
export class InterlinkedGuild {
  ...
  async listGames(interaction, gameList) {
    ...
    for (let i = 0; i != messages.length; i++) {
      ...
      if (response.id == null) return await this.#rest.createInteractionResponse(interaction, `{"type":4,"data":{"flags":${1 << 6},"content":"Couldn't fully send the game list. Are you sure you gave proper message permissions to me? :thinking:"}}`);
      if (response.channel_id == null) return await this.#rest.createInteractionResponse(interaction, `{"type":4,"data":{"flags":${1 << 6},"content":"Couldn't fully send the game list, are you sure you gave proper message permissions to me? :thinking:"}}`);
      this.#gameListChannelID = response.channel_id;
      this.#gameListMessageIDs.push(response.id);
    }
    await this.#rest.createInteractionResponse(interaction, `{"type":4,"data":{"flags":${1 << 6},"content":"Live games list will be posted here. 🎮\\nNote that if there are any changes then the games list messages will be deleted and sent again with the latest info."}}`);
    return response;
  }
...
export class Interlinked {
  ...
  constructor(
    ...
    this.#serverRequestURL = Interlinked.getServerRequestURL(config.steamAPIKey, config.steamAppID, config.steamGameDir);
  ...
  static getServerRequestURL(steamAPIKey, steamAppID, steamGameDir) {
    var _filter = '';
    if (steamAppID != null) {
      if (steamAppID != '') {
  ...
  _updateServerRequestURL() {
    this.#serverRequestURL = Interlinked.getServerRequestURL(this.#steamAPIKey, this.#steamAppID, this.#steamGameDir);
  }
  ...
  async listGames(interaction) {
    return this.#discord.listGames(interaction, await InterlinkedGameList.fromURL());
  }

  async updateGameLists() {
    return this.#discord.updateGameLists(await InterlinkedGameList.fromURL());
  }
