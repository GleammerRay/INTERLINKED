...
export class InterlinkedGuild {
  ...
  buildServerListMessage(serverList, image = null, showInactive = false) {
  ...
  async updateServerList(serverList, image = '') {
    if (this.#serverListMessageID != null) {
      var _message = this.buildServerListMessage(serverList, image, true);
  ...
  async getServers() {
    ...
    _servers.response.servers.forEach((server) => {
      _serverDataRequests.push({
        request: queryServerInfo(server.addr),
        server: server,
      });
    });
