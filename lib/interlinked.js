...
'{' +
  `"name":"remove","type":1,"description":"${removeDesc}","default_member_permissions":"0","default_permission":false,"dm_permission":false,"options":[` +
    `{"name":"ping_role","type":1,"default_member_permissions":"0","default_permission":false,"dm_permission":false,"description":"${pingRoleDesc}"}` +
  ']' +
'},' +
'{' +
...
export class InterlinkedGuild {
  ...
  _getBlacklistedServerList(serverList) {
    serverList = new InterlinkedServerList(
      Array.from(serverList.activeServers),
      serverList.serverCount,
      serverList.playerCount
    );
    for (var i = 0; i != serverList.activeServers.length; i++) {
      var _server = serverList.activeServers[i];
      if (this.#blacklist.includes(_server.address)) {
        serverList.activeServers.splice(i, 1);
        serverList.serverCount--;
        serverList.playerCount -= _server.playerCount;
        continue;
      }
    }
    return serverList;
  ...
  resetVariables(interaction) {
    if (!isAdmin(interaction.member.permissions)) return this.sendPermissionDenied(interaction);
...
export class Interlinked {
  ...
  listHelp(interaction) {
    var _isAdmin = isAdmin(interaction.member.permissions);
  ...
  async executeInteraction(interaction) {
    ...
    switch (interaction.data.name) {
      case 'help':
        return this.listHelp(interaction);
      case 'pub':
      ...
      case 'reset':
        return this.#discord.resetVariables(interaction);
      case 'set':        
        return this.#discord.setVariable(interaction);
      case 'remove':
        return this.#discord.removeVariable(interaction);
      case 'subscribe':
        return this.#discord.subscribeChannel(interaction);
      case 'unsubscribe':
        return this.#discord.unsubscribeChannel(interaction);
      case 'blacklist':
        return this.#discord.blacklist(interaction);       
      case 'unblacklist':
        return this.#discord.unblacklist(interaction);
      default:
        return _onLog(`WARNING:Unknown command:${interaction.data.name}`);
    }   
  }
