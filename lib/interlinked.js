...
export class InterlinkedGuild {
  ...
  postServerListInteractionResponse(interaction, serverList, image = null) {
    ...
    if (this.shouldPing(serverList)) {
      this.delayNextPing();
      return this.#rest.createMessage(interaction.channel_id, `{"content":"<@&${this.#pingRole}> :beers:"}`);
    }
    var _message = this.buildServerListMessage(serverList, image);
    return this.#rest.createInteractionResponse(interaction, `{"type":4,"data":${_message}}`);
  }
  ...
  async addUserPingRole(interaction) {
    ...
    if (this.#pingRole == null) {
      return await this.#rest.createInteractionResponse(interaction, '{"type":4,"data":{"content":"Ping role is not set up. :no_mouth:\\nUse `/set ping_role` first"}}');
    }
    if (interaction.member.roles.indexOf(this.#pingRole) != -1) {
      return await this.#rest.createInteractionResponse(interaction, '{"type":4,"data":{"content":"You already have the ping role. :upside_down:"}}');
    }
    var _response = await this.#rest.addGuildMemberRole(interaction.guild_id, _user.id, this.#pingRole);
    if (_response.code == 50001) {
      return await this.#rest.createInteractionResponse(interaction, '{"type":4,"data":{"content":"No access to changing member roles. :lock:"}}');
    }
    if (_response.code == 50013) {
      return await this.#rest.createInteractionResponse(interaction, '{"type":4,"data":{"content":"Can\'t manage a role that\'s higher than mine. Move me up the list! :baby:"}}');
    }
    return await this.#rest.createInteractionResponse(interaction, '{"type":4,"data":{"content":"You\'ll never miss a game! :beer: :superhero: :mega:"}}');
  }
  ...
  async removeUserPingRole(interaction) {
    ...
    if (interaction.member.roles.indexOf(this.#pingRole) == -1) {
      return await this.#rest.createInteractionResponse(interaction, '{"type":4,"data":{"content":"You don\'t have a ping role. :deaf_person:"}}');
    }
    ...
    return await this.#rest.createInteractionResponse(interaction, '{"type":4,"data":{"content":"You will no longer be pinged. :sleeping:"}}');
  }
  ...
  resetVariable(interaction) {
    ...
    switch (_varName) {
      ...
      case 'canPing':
        ...
        break;      
      default:
        this._onLog(`W:Request to remove unknown variable:${_varName}`);
        return;
    }
  ...
  removeVariable(interaction) {
    ...
    var _var = interaction.data.options[0];
    var _varName = snakeToCamelCase(_var.name);
    this._onLog(`I:Removing variable '${_varName}' for guild ${interaction.guild_id}.`);
    this[_varName] = null;
...
export class Interlinked {
  ...
  async _onMessage(event) {
      ...
      var _msg = `I:Discord gateway sent Opcode ${_data.op} `;
      switch (_data.op) {
        case 0:
          _msg = _msg + _data.t;
        ...
        case 10:
          ...
          this._onLog('I:Sending Opcode 2 Identify to Discord.');
          if (!this.#gateway.trySend(`{"op":2,"d":{"token":"${this.#botToken}","intents":512,"properties":{"os":"linux","browser":"INTERLINKED","device":"INTERLINKED"}}}`)) {
  ...
  async _onClose(event) {
    const _closeMsg = 'I:Discord gateway connection closed.';
    if (event.reason != '') {
