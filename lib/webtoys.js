...
export function buildServerField(server) {
  ...
  var result =
`    <p2 class="embed-topserver-name">${emoji} ${server.name.replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;')}</p2>
    <br>
    ...
    <p2 class="code embed-topserver-value">${server.mapName.replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;')}</p2>
    <br>`;
  return result;
}
